...
# ---- 3.2: Tract-level demographics (percentages only) ----
...
# 2) Convert 5-digit county GEOIDs to 3-digit county FIPS
county_codes3 <- stringr::str_sub(selected_counties$GEOID, -3, -1)

# (optional safety check)
stopifnot(length(county_codes3) > 0, all(nchar(county_codes3) == 3))

# 3) Pull tract-level ACS for those counties
tract_raw <- get_acs(
  geography = "tract",
  variables = vars_tracts,
  year      = acs_year,   # you defined acs_year <- 2022 above
  survey    = "acs5",
  state     = my_state,
  county    = county_codes3,   # <-- 3-digit FIPS codes here
  output    = "wide"
)

# 4) Clean tract/county names and compute percentages
# NAME is like: "Census Tract 123, Kings County, New York"
tract_demo <- tract_raw %>%
  tidyr::separate(NAME, c("tract_label","county_label","state_label"),
                  sep = ", ", remove = FALSE) %>%
  dplyr::mutate(
    tract_label  = stringr::str_remove(tract_label, "^Census Tract\\s*"),
    county_label = stringr::str_remove(county_label, "\\s*County$"),
...
top_hispanic_tract <- tract_demo %>%
  ...
  dplyr::slice(1) %>%
  dplyr::select(GEOID, tract_label, county_label,
...
knitr::kable(county_summary_weighted,
             digits = 1,
             caption = "County-wide demographics (population-weighted)")

```
```{r}
# ---- 3.3: Weighted county-level demographics ----
...
tract_moe <- tract_demo %>%
  dplyr::mutate(
    # MOE% for each group
    white_moe_pct    = ifelse(white_nhE > 0, 100 * white_nhM / white_nhE, NA),
    black_moe_pct    = ifelse(black_nhE > 0, 100 * black_nhM / black_nhE, NA),
    hispanic_moe_pct = ifelse(hispanicE > 0, 100 * hispanicM / hispanicE, NA),

    # flag if any group’s MOE% > 15
    high_moe_flag = (white_moe_pct > 15) | (black_moe_pct > 15) | (hispanic_moe_pct > 15)
  )

# summary of how many tracts have high MOE issues
tract_moe_summary <- tract_moe %>%
  dplyr::summarise(
    total_tracts = dplyr::n(),
    high_moe_tracts = sum(high_moe_flag, na.rm = TRUE),
    pct_high_moe = 100 * high_moe_tracts / total_tracts
  )

knitr::kable(tract_moe_summary,
             digits = 1,
...
```
```{r}
# ---- 4.2: Count flags per group by county ----

flags_by_county <- tract_moe2 %>%
  dplyr::group_by(county_label) %>%
  dplyr::summarise(
...
knitr::kable(
  flags_by_county,
  caption = "Number of tracts with MOE% > 15% for each group, by county"
)
