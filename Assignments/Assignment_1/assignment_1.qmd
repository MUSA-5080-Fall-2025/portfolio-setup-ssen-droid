...
**Comment on the output:** Only one county in New York is low confidence with regard to MOE - Hamilton, NY. This is likley due to extremely small population size coupled with small sample perhaps. 
...
tract_demo <- tract_raw %>%
  dplyr::mutate(
    pct_white_nh = ifelse(total_popE > 0, 100 * white_nhE / total_popE, NA),
    pct_black_nh = ifelse(total_popE > 0, 100 * black_nhE / total_popE, NA),
    pct_hispanic = ifelse(total_popE > 0, 100 * hispanicE / total_popE, NA)
  ) %>%
  dplyr::mutate(
    tract_label  = stringr::str_extract(NAME, "^Census Tract\\s*[^,;]+"),
    county_label = NAME %>%
      stringr::str_extract("[,;]\\s*[^,;]+\\s*County") %>%
      stringr::str_replace_all("^[,;]\\s*|\\s*County$", "") %>%
      stringr::str_trim()
  )
